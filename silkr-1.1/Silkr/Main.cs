...
namespace Silkr {
    public partial class Main : Form {
        ...
        private void VirtualCursorUpdate(ref Message m) {
            ...
            if (isDrawing) {
                if (tabletMode) {
                    ...
                    NOT WORKING ||||||
                    ******************/
                    // Point p = new Point(tabletX + virtualLeft, tabletY + virtualTop);
                    position = p;
                    overlay.cursorPos = p;
                    overlay.Invalidate();
        ...
        private void ToolStripMenuItem_saveConfig_Click(object sender, EventArgs e) {
            config.SaveConfig();
            MessageBox.Show("Configuration settings saved to: Silkr.config", "Silkr", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }

        private void ToolStripMenuItem_restoreDefaults_Click(object sender, EventArgs e) {
            config.LoadConfig(true);
            MessageBox.Show("Default settings restored.", "Silkr", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
